...
/*   Updated: 2023/11/08 16:50:32 by svalente         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_aux(char delim, char *str, int i)
{
	if (delim == '|' && (str[i] == '<' || str[i] == '>'))
		return (1);
...
int	checker(char *rl)
{
	if (!valid_quotes(rl))
	{
		printf("syntax error: invalid quotes\n");
		free (rl);
		return (0);
	}
